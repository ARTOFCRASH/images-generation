...
int main(int argc, char** argv) {
    ...
    std::cerr << "after load, points: " << point_cloud.size() << std::endl;
    ...
    ne.setKSearch(500);
    std::cerr << "start normal estimation" << std::endl;
    ne.compute(*cloud_normals);
    std::cerr << "after normal estimation" << std::endl;
    ...
    std::cerr << "start segmentation" << std::endl;
    // Obtain the plane inliers and coefficients
    seg.segment(*inliers_plane, *coefficients_plane);
    std::cerr << "after segmentation" << std::endl;
    ...
    std::cerr << "start viewer" << std::endl;
...
//   pcl::transformPointCloud (*cloud_persimmon, *yawed_cloud, yaw_matrix);
 // ---------------- Timer Start ----------------
    ...
    std::cout << "[Start] " << std::put_time(&start_tm, "%Y-%m-%d %H:%M:%S") << std::endl;
...
// }
    ...
    std::cout << "[End] " << std::put_time(&end_tm, "%Y-%m-%d %H:%M:%S") << std::endl;

    auto duration = std::chrono::duration_cast<std::chrono::seconds>(end_time - start_time).count();
    std::cout << "[Total] " << duration / 3600 << " hours "
